...
export default function GeneratePage() {
  ...
  const handleGenerate = async () => {
    ...
    const newWork: Work = {
      ...
      ogMeta: {
        ...
        desc: `${state.user?.nickname}のAiconが生成したオリジナル${todayTheme.theme}`,
  ...
  const generateMockLyrics = (theme: string) => ({
    aMelody: [`${theme}の中で`, '君を思い出してる', 'あの日の約束', '今も胸に響いて'],
    chorus: ['時は過ぎても', '変わらない想い', '君への愛を', '歌い続けよう']
  });
  ...
  if (isGenerating) {
    return (
      <div className="min-h-screen flex items-center justify-center px-4">
        <LoadingAnimation message="作品を生成中..." type="creation" />
      </div>
    );
  }
      ...
      <div className="min-h-screen bg-gradient-to-br from-yellow-100 via-orange-50 to-pink-100 px-4 py-8">
        <div className="max-w-2xl mx-auto space-y-8">
          ...
          <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="text-center space-y-4">
            ...
            <h1 className="text-3xl font-bold gradient-text">作品が完成しました！</h1>
            <p className="text-gray-600">あなたのAiconが素晴らしい作品を創造しました</p>
          </motion.div>
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <h2 className="text-xl font-semibold text-gray-800">{generatedWork.ogMeta.title}</h2>
                    <p className="text-sm text-gray-500">{new Date(generatedWork.createdAt).toLocaleString('ja-JP')}</p>
                  </div>
                </div>
              ...
              <div className="flex items-center justify-center space-x-4 pt-4 border-t">
                <Button onClick={handleLike} variant="outline" size="sm" className="flex items-center space-x-2">
                  <span>❤️</span>
                ...
                <Button onClick={() => router.push('/works')} variant="outline" size="sm">
                  作品一覧を見る
                </Button>
              </div>
            </div>
          ...
          <AnimatedCard delay={0.5}>
            <div className="text-center space-y-4">
              <h3 className="text-lg font-semibold text-gray-800">Aiconの成長</h3>
          ...
          <div className="text-center space-y-4">
            ...
            <p className="text-sm text-gray-500">素晴らしい作品をありがとうございます！</p>
          </div>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-gradient-to-br from-yellow-100 via-orange-50 to-pink-100 px-4 py-8">
      <div className="max-w-2xl mx-auto space-y-8">
        ...
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="text-center space-y-4">
          ...
          <h1 className="text-3xl font-bold gradient-text">{todayTheme.theme}を生成</h1>
          <p className="text-gray-600">今日のテーマ: {todayTheme.title}</p>
        </motion.div>

        {/* Clone Preview */}
        <AnimatedCard delay={0.2} glow>
          <div className="text-center space-y-4">
            <h2 className="text-xl font-semibold text-gray-800">あなたのAicon</h2>
            <div className="flex justify-center">
              <CloneAvatar look={state.clone?.look || {}} size="lg" showDetails animated />
            </div>
        ...
        <AnimatedCard delay={0.3}>
          <div className="space-y-4">
            <h3 className="text-lg font-semibold text-gray-800">生成に必要な条件</h3>
            <div className="space-y-2">
              ...
              >
                ...
                <span className="text-gray-700">Q&A回答 ({state.todaysProgress.qaCompleted}/3)</span>
              </div>
              ...
              >
                ...
                <span className="text-gray-700">ミニゲーム完了</span>
              </div>
            </div>

            {!canGenerate && (
              <div className="p-4 bg-yellow-50 rounded-2xl">
                <p className="text-yellow-700 text-sm">💡 Q&Aかミニゲームを完了すると作品生成が可能になります</p>
              </div>
            )}
          </div>
        </AnimatedCard>
        ...
        <div className="text-center space-y-6">
          ...
          {canGenerate && <p className="text-sm text-gray-500">生成には約30秒かかります</p>}
        </div>
