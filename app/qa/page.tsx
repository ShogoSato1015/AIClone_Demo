...
export default function QAPage() {
  ...
  const handleSubmit = async () => {
    ...
    const pointsReward = parseInt(urlParams.get('points') || '2');
    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-4 bg-gradient-to-br from-emerald-100 via-teal-50 to-blue-100">
        ...
        >
          ...
          <h2 className="text-3xl font-bold text-green-800">今日のQ&Aは完了済みです</h2>
      ...
      <div className="min-h-screen flex items-center justify-center px-4">
        <div className="text-center space-y-6">
          ...
          <div className="space-y-2">
            ...
            <p className="text-gray-600">あなたの回答からAiconが学んでいます</p>
          </div>
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-pink-50 px-4 py-8">
      <div className="max-w-2xl mx-auto space-y-8">
        ...
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="text-center space-y-4">
          ...
          <h1 className="text-3xl font-bold text-gray-800">Aiconに学習させよう</h1>
          <p className="text-gray-600">あなたの回答がAiconの個性を育てます</p>
        </motion.div>

        {/* Progress */}
        <ProgressBar current={currentQuestionIndex + 1} total={questions.length} color="primary" label="質問の進捗" />
              ...
              <div className="space-y-6">
                <div className="flex items-start space-x-4">
                  ...
                  <div className="flex-1">
                    <h2 className="text-xl font-semibold text-blue-800 mb-2">{currentQuestion?.label}</h2>
                  </div>
                </div>
                    ...
                    {currentQuestion.choices.map((choice, index) => (
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <span>{choice}</span>
                          {answers[currentQuestion.id] === choice && <span className="text-blue-600 text-xl">✓</span>}
                        </div>
                      </motion.button>
                    ))}
                  </div>
                ...
                {currentQuestion?.type === 'text' && (
                  <div className="space-y-3">
                    <textarea
                      ...
                      maxLength={200}
                    />
                    <p className="text-xs text-gray-500 text-right">{(answers[currentQuestion.id] || '').length}/200</p>
                  </div>
                )}
                ...
                {answers[currentQuestion.id] && (
                  ...
                  >
                    <CloneAvatar look={state.clone?.look || {}} size="sm" animated={false} />
                    <div className="text-sm text-gray-600">
                      <span className="font-medium">Aicon:</span> {getCloneReaction(answers[currentQuestion.id])}
                    </div>
                  </motion.div>
                )}
              </div>
        ...
        <div className="flex justify-between items-center">
          ...
          <Button onClick={handleNext} variant="primary" disabled={!canProceed}>
        ...
        <div className="text-center space-y-4">
          <p className="text-sm text-gray-500">💡 正解はありません。素直な気持ちで答えてくださいね</p>
